...
class doneWithTheEarth:
    ...
    def calculatedata(self):
        self.global_cur.execute("SELECT iso_a3,emissions FROM Emissions_Data where emissions = (SELECT MAX(emissions) FROM Emissions_Data)")
        highest = self.global_cur.fetchone()
        self.global_cur.execute("SELECT iso_a3,emissions FROM Emissions_Data where emissions = (SELECT MIN(emissions) FROM Emissions_Data)")
